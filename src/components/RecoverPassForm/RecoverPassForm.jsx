...
const RecoverPassForm = ({ token }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "password" && data.passwordValidation.length > 0) {
      validateField(
        "passwordValidation",
        updatedData.passwordValidation,
        updatedData
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const isPasswordValidation = validateField(
      "passwordValidation",
    ...
    try {
      ...
      setSuccessMessage(
        "¡Cambio de contraseña con éxito! Serás redirigido al Inicio"
      );
      ...
      setErrorMessage("Error: ¡TOKEN EXPIRADO!");
      ...
      <div className="form-container-recover">
        ...
        <section className="form-recover-pass">
          ...
          <InputPassword
            ...
            areErrors={
              errors.passwordValidation && data.passwordValidation.length > 0
            }
          />
          {errors.passwordValidation && data.passwordValidation.length > 0 ? (
            <span className="error-message">{errors.passwordValidation}</span>
          ) : (
            ""
          )}
        </section>
